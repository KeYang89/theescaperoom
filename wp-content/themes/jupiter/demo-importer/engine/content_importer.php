...
class ContentImporter
{
    ...
    function __construct($atts)
    {
        ...
        $this->theme_content_path        = BASE_PATH . $options['template'] . '/' . THEME_CONTENT_FILE_NAME;
    ...
    private function mk_process_theme_options_import()
    {
        if ($this->is_options == 'true') {
            ...
            if (!empty($import_data_unserilized)) {
                update_option(THEME_OPTIONS, $import_data_unserilized);
    ...
    private function mk_import_data($data)
    {
        ...
        if (empty($data) || !is_object($data)) {
            wp_die(__('Import data could not be read. Please try a different file.', 'widget-importer-exporter'), '', array(
                'back_link' => true
            ));
        }
        $results = array();
        foreach ($data as $sidebar_id => $widgets) {
            ...
            foreach ($widgets as $widget_instance_id => $widget) {
                ...
                $results[$sidebar_id]['widgets'][$widget_instance_id]['title']        = $widget->title ? $widget->title : __('No Title', 'widget-importer-exporter');
