		...
		function add_admin_header_membershippings() {
			...
			$factory = Membership_Plugin::factory();
		        ...
		        function handle_members_panel()
		        {
		            ...
		            $subs = $this->get_subscriptions(array(
		                'sub_status' => 'active'
		            ));
		...
		?>
					...
					<form method="get" action="?page=<?php echo esc_attr($page);?>" id="members-filter">
						...
						<div class="tablenav">
							...
							<div class="alignleft actions">
								...
								<?php if (!empty($subs)): ?>
									<select name="sub_op" style='float:none;'>
										...
										<?php foreach ($subs as $key => $sub) { ?>
											...
											<?php if (isset($_GET['sub_op']) && $_GET['sub_op'] == $sub->id)
													echo 'selected="selected"'; ?>>
											<?php echo esc_html($sub->sub_name); ?>
										</option>
										<?php } //end foreach ?>
									</select>
								...
								<select name="level_op" style='float:none;'>
									    ...
									    if ($levels) {
									        foreach ($levels as $key => $level) { ?>
												<option value="<?php echo $level->id;?>"
													<?php if (isset($_GET['level_op']) && $_GET['level_op'] == $level->id) echo 'selected="selected"';?>>
									...
									<select name="active_op" style='float:none;'>
										...
										<option value="yes"
											<?php if (isset($_GET['active_op']) && $_GET['active_op'] == 'yes') echo 'selected="selected"'; ?>>
											<?php _e('Active', 'membership'); ?>
										</option>
										<option value="no"
											<?php if (isset($_GET['active_op']) && $_GET['active_op'] == 'no') echo 'selected="selected"'; ?>>
											<?php _e('Inactive', 'membership'); ?>
										</option>
									</select>
									<input type="submit" class="button-secondary action" id="doactionactive" name="doactionactive" value="<?php _e('Filter', 'membership'); ?>">
							</div>  <?php // END <div class="alignleft actions"> // ?>
							...
							<br class="clear">
						</div> <?php // END <div class="tablenav"> // ?>

						<?php if (is_wp_error($wp_user_search->search_errors)): ?>
							<div class="error">
								<ul>
								<?php
									foreach ($wp_user_search->search_errors->get_error_messages() as $message) echo "<li>$message</li>";
								?>
								</ul>
							</div>
						<?php endif; ?>
								...
								<tr>
									...
									<?php
					    				foreach ($columns as $key => $col) {
					        				if ($key == 'sub' && empty($subs)) { continue; }
											?>
					    		...
					    		foreach ($wp_user_search->get_results() as $user) {
									...
									<tr id='user-<?php echo $user_object->ID; ?>' <?php echo $style; ?>>
										...
										<td <?php echo $style; ?>>
											...
											<div class="row-actions"><?php echo (!empty($userlevels)) ? implode(" | ", $actions) : ''; ?></div>
										</td>
										...
										<?php // GATEWAY COLUMN // ?>
										<td <?php echo $style; ?>>
										<?php $usersubs = $user_object->get_relationships();
					        				//print_r($subs);
					        				if ($usersubs) {
									            $gates = array();
									            foreach ($usersubs as $sub) {
									                if ($sub->usinggateway != 'admin') {
									                    ...
									                    $gates[] = is_object($gateway) ? $gateway->title : sprintf('<i>%s</i><!-- %s -->', esc_html__('not found or deactivated', 'membership'), $sub->usinggateway);
									                } else {
						...
						<div class="tablenav">
							...
							<div class="alignleft actions">
								...
								<select name="sub_op2">
									...
									<?php
									    ...
									    if ($subs) {
									        foreach ($subs as $key => $sub) { ?>
												<option value="<?php echo $sub->id; ?>"
												<?php if (isset($_GET['sub_op2']) && $_GET['sub_op2'] == $sub->id) echo 'selected="selected"'; ?>>
												<?php echo esc_html($sub->sub_name); ?></option>
												<?php
					        				} // end foreach
									    }
									?>
								...
								<select name="level_op2">
									...
									<?php
					    				...
					    				if ($levels) {
									        foreach ($levels as $key => $level) { ?>
												<option value="<?php echo $level->id; ?>"
													<?php if (isset($_GET['level_op2']) && $_GET['level_op2'] == $level->id) echo 'selected="selected"'; ?>>
													<?php echo esc_html($level->level_title);?></option>
												<?php
					        				} //end foreach
									    }
									?>
								...
								<select name="active_op2">
									<option value=""><?php _e('Filter by status', 'membership'); ?></option>
									<option value="yes" <?php if (isset($_GET['active_op2']) && $_GET['active_op2'] == 'yes') echo 'selected="selected"'; ?>>
										<?php _e('Active', 'membership'); ?>
									</option>
									<option value="no" <?php if (isset($_GET['active_op2']) && $_GET['active_op2'] == 'no') echo 'selected="selected"'; ?>>
										<?php _e('Inactive', 'membership'); ?>
									</option>
								</select>
				...
				function handle_options_panel_updates() {
						...
						if ($action == 'updateoptions') {
								...
								switch ($tab) {
										...
										case 'configuration':
												$M_options['protect_front_page'] = (isset($_POST['protect_front_page'])) ? $_POST['protect_front_page'] : '';
												$M_options['paymentcurrency'] = (isset($_POST['paymentcurrency'])) ? $_POST['paymentcurrency'] : '';
												...
												$M_options['allow_page_rule_cascade'] = (isset($_POST['allow_page_rule_cascade'])) ? $_POST['allow_page_rule_cascade'] : '';
				...
				function show_configuration_options() {
						...
						<div id="poststuff" class="metabox-holder m-settings">
								<form action='' method='post'>
										...
										<div class="postbox">
											<h3 class="hndle" style='cursor:auto;'><span><?php _e('Front Page', 'membership'); ?></span></h3>
											<div class="inside">
												<p class='description'><?php _e('By default your site home page will always display. Check the following box if you would like to protect your home page with rules.', 'membership'); ?></p>

												<table class="form-table">
													<tbody>
													<tr valign="top">
														<th scope="row"><?php _e('Apply rules to site home/front page', 'membership'); ?></th>
														<td>
															<?php
															$protect_front_page = (isset($M_options['protect_front_page'])) ? $M_options['protect_front_page'] : 'no';
															?>
															<input type='checkbox' name='protect_front_page' value='yes' <?php if ($protect_front_page == 'yes') echo "checked='checked'"; ?>/>
														</td>
													</tr>
													</tbody>
												</table>
										...
										<div class="postbox">
												<h3 class="hndle" style='cursor:auto;'><span><?php _e('Page Rules', 'membership'); ?></span></h3>
												<div class="inside">
														<p class='description'><?php _e('Use the following option to allow a parent page\'s access rules to apply to it\'s child pages.', 'membership'); ?></p>

														<table class="form-table">
																<tbody>
																		<tr valign="top">
																				<th scope="row"><?php _e('Cascade Page Rules', 'membership'); ?></th>
																				<td>
																						<?php
																						$allow_page_cascade = (isset($M_options['allow_page_rule_cascade'])) ? $M_options['allow_page_rule_cascade'] : 'yes';
																						?>
																						<input type='checkbox' name='allow_page_rule_cascade' value='yes' <?php if ($allow_page_cascade == 'yes') echo "checked='checked'"; ?>/>
																				</td>
																		</tr>
																</tbody>
														</table>
				...
				function handle_sub_edit_form($sub_id = false, $clone = false) {
						...
						?>
								...
								<div class='sub-liquid-left'>
										...
										<input type="text" class="wide" name="sub_order_num" value="<?php echo !empty( $sub->order_num ) ? intval( $sub->order_num ) : 0 ?>">
